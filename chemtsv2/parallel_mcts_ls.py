...
class p_mcts:
    ...
    def __init__(self, communicator, chem_model, reward_calculator, conf, logger):
        ...
        # used for async simulation
        self.sim_task_queue = asyncio.Queue()
        self.sim_result_queue = asyncio.Queue()
    ...
    def MP_MCTS(self):
        ...
        async def _proc_search(tag, message):
            ...
            if self.hsm.search_table(message[0]) == None:
                ...
                if node.state == ['&']:
                    ...
                    if len(node.state) < node.max_len:
                        gen_id = self.get_generated_id()
                        await self.sim_task_queue.put((node, gen_id, False))
                    else:
                        score = -1
                        node.update_local_node(score)
                        self.hsm.insert(Item(node.state, node))
                ...
                if node.state == ['&']:
                    ...
                    if len(node.state) < node.max_len:
                        if node.state[-1] != '\n':
                            ...
                            gen_id = self.get_generated_id()
                            await self.sim_task_queue.put((node, gen_id, True))
                    else:
                        score = -1
                        node.update_local_node(score)
                        self.hsm.insert(Item(node.state, node))
        ...
        async def _proc_backprop(tag, message):
            node = Tree_Node(state=message[0], reward_calculator=self.reward_calculator, conf=self.conf)
            node.reward = message[1]
            local_node = self.hsm.search_table(message[0][0:-1])
            if local_node.state == ['&']:
        ...
        async def _proc_checkpoint_ready(tag, message):
            assert self.rank == 0
        ...
        async def _proc_checkpoint_save(tag, message):
            ...
            while not timeup:
                ...
                if jobq_non_empty:
                    ...
                    if tag == JobType.SEARCH.value:
                        ...
                        await _proc_checkpoint_prepare(tag, message)
                    elif tag == JobType.CHECKPOINT_READY.value:
                        await _proc_checkpoint_ready(tag, message)
        ...
        async def _main_loop():
            await asyncio.gather(_probe_loop(), _async_simulation())
