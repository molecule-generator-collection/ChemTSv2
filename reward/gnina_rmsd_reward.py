...
class Gnina_RMSD_reward(Reward):
    def get_objective_functions(conf):
        def GninaScore(mol):
            ...
            scaffold_rmsd = calc_scaffold_rmsd(top_CNNpose_mol, conf)
            ...
            return [smina_affinity, cnn_score, cnn_affinity, scaffold_rmsd]

        return [GninaScore]

    def calc_reward_from_objective_values(values, conf):
        smina_affinity, cnn_score, cnn_affinity, scaffold_rmsd = values[0]
        if scaffold_rmsd > conf["rmsd_threshold"] or scaffold_rmsd is None:
            return -1
