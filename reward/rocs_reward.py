...
class ROCS_reward(Reward):
    def get_objective_functions(conf):
        def RocsScore(mol):
            ...
            openeye_out_dir = os.path.join(conf['output_dir'], "omega_rocs_result", f"gid_{conf['gid']}")
            ...
            cmd_omega = [
                ...
                "-in", gen_mol_fname,
                "-out", omega_db_fname,
                "-maxconfs", str(conf['omega']['maxconfs']),
                "-strictstereo", "false"]
            try:
            ...
            cmd_rocs = [
                "rocs",
                "-query", conf['rocs']['query'],
                "-mcquery",
                "-dbase", omega_db_fname]
            try:
