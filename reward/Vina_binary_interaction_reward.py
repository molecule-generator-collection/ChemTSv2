    ...
    def get_objective_functions(conf):
        def VinaScore(mol):
            ...
            cmd = [
                ...
                temp_ligand_fname,
                "--center_x",
                str(conf["vina_center"][0]),
                "--center_y",
                str(conf["vina_center"][1]),
                "--center_z",
                str(conf["vina_center"][2]),
                "--size_x",
                str(conf["vina_box_size"][0]),
                "--size_y",
                str(conf["vina_box_size"][1]),
                "--size_z",
                str(conf["vina_box_size"][2]),
                "--cpu",
                str(conf["vina_cpus"]),
                "--exhaustiveness",
                str(conf["vina_exhaustiveness"]),
                "--max_evals",
                str(conf["vina_max_evals"]),
                "--num_modes",
                str(conf["vina_num_modes"]),
                "--min_rmsd",
                str(conf["vina_min_rmsd"]),
                "--energy_range",
                str(conf["vina_energy_range"]),
                "--out",
                output_ligand_fname,
                "--spacing",
                str(conf["vina_spacing"]),
                "--verbosity",
                str(verbosity),
            ]
            if conf["debug"]:
            ...
            if not conf["debug"]:
                shutil.rmtree(temp_dir)
            min_distance_dict = get_interaction_distances(
                conf["prolif_receptor"], output_ligand_fname, 0, conf
            )
