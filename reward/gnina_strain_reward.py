...
class Gnina_strain_reward(Reward):
    def get_objective_functions(conf):
        def GninaScore(mol):
            ...
            total_strain_energy, max_single_strain_energy = calc_strain_energy(output_ligand_fname, conf)
            if conf['debug']:
            ...
            return [smina_affinity, cnn_score, cnn_affinity, total_strain_energy, max_single_strain_energy]
        return [GninaScore]
    
    
    def calc_reward_from_objective_values(values, conf):
        smina_affinity, cnn_score, cnn_affinity, total_strain_energy, max_single_strain_energy = values[0]
