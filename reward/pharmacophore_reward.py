...
def create_pharmacophore():
    ...
    cluster_centers_sel = {
        ...
        "hydrophobics": [[-14.0335, 18.17425, -21.44525]],
        "aromatics": [[-12.645999999999999, 14.753833333333333, -28.08808333333333]],
    }
    type_dict = {
        "donors": "Donor",
        "acceptors": "Acceptor",
    ...
    for ptype, coords in cluster_centers_sel.items():
        for coord in coords:
            feat = ChemicalFeatures.FreeChemicalFeature(
                type_dict[ptype], Geometry.Point3D(coord[0], coord[1], coord[2])
            )
    ...
    for i, j in zip(*np.where(np.tril(bmatrix) > 0)):
        i_type = pcophore.getFeature(i).GetFamily()
        j_type = pcophore.getFeature(j).GetFamily()
        pcophore.setLowerBound(
            i, j, max(bmatrix[i, j] - (radi_dict[i_type] + radi_dict[j_type]), 0)
        )
    # print(pcophore)
    ...
    def get_objective_functions(conf):
        def PharmacophoreSearch(mol):
            ...
            if can_match:
                ...
                match_list = [
                    f"{k}:{lig_feat_counter[k]}:{ph4_feat_counter[k]}"
                    for k in ph4_feat_counter.keys()
                ]
            else:
                lig_feat_counter = collections.Counter([
                    f.GetFamily() for f in FEATURE_FACTORY.GetFeaturesForMol(mol)
                ])
                match_list = [
                    f"{k}:{lig_feat_counter[k]}:{ph4_feat_counter[k]}"
                    for k in ph4_feat_counter.keys()
            ...
            failed, _, matched, _ = EmbedLib.MatchPharmacophore(
                all_matches, bm, PCOPHORE, useDownsampling=False
            )
            if failed:
                return [match_list, None]
