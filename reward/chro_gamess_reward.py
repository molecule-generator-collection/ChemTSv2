...
class UV_reward(Reward):
    def get_objective_functions(conf):
        ...
        def UV(mol):
            ...
            try:
                run_pack = GamessRunPack.GamessDFTRun(
                    conf['gms_functional'],
                    conf['gms_basis'], 
                    conf['gms_core_num'],
                    conf['gms_option'],
                    sdf_input_opt,
                    error=str(conf['gms_error']))
                run_pack.mem = conf['gms_memory']  # 5GB minimum
                result_dict = run_pack.run_gamess()
                run_pack = None  # 
            except Exception as e:
                print('Gamess DFT calculation failed:', e)
                result_dict = {}
            os.chdir(current_dir)

            #Post-processing
            #If Gamess failed and did not make calc_dir, calc_dir for gamess is required for the later process.
            if not os.path.exists(calc_dir):
                os.mkdir(calc_dir)
            result_base_dir = os.path.join(conf['output_dir'], 'gamess_result')
