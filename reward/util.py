...
def calc_stereo_centor(smiles, reaction_a_list, reaction_aa_list, ss_list, mol_type):
    ...
    include_flag = False
    num_match_sum = 0
    for i, (reaction_a, reaction_aa, ss) in enumerate(
        zip(reaction_a_list, reaction_aa_list, ss_list)
    ):
        try:
...
def calc_3dstructure(smiles):
    ...
    mol_list, ss_flag = calc_stereo_centor(
        smiles, reaction_a_list, reaction_aa_list, ss_list, mol_type
    )
    ...
    if mol_type == "smiles" or mol_type == "skip" or charge_error:
        ...
        for i, smi in enumerate(mol_list):
            ...
            try:
                ...
                try:
                    pybel_out = subprocess.run(
                        command0, capture_output=True, text=True, timeout=threshold_time
                    ).stdout
                    file_list.append(no3d_file)
                    pybel_out = subprocess.run(
                        command1, capture_output=True, text=True, timeout=threshold_time
