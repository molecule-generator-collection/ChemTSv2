...
If you want to use the RNN models trained on your own datasets, use `chemtsv2-train-rnn` command and [config/rnn_model_setting.yaml](config/rnn_model_setting.yaml) to train the models.
You need to prepare a dataset that only contains SMILES string and modify the path in `dataset` key in `rnn_model_setting.yaml`. And then, run the following command:

```bash
chemtsv2-train-rnn -c config/rnn_model_setting.yaml
```

Please note that the current version of ChemTSv2 does not support the change for RNN model structures, and users can only change the parameters described in `rnn_model_setting.yaml`.
