...
def test_debug_check_success(setup_and_cleanup):
    ...
    result02 = subprocess.run(
        ["chemtsv2-debug-check"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True
    )
    assert result02.returncode == 0, f"Expected return code 0 but got {result02.returncode}"
    assert "[INFO] Output validation successful" in result02.stdout, (
        f"Expected '[INFO] Output validation successful' but got {result02.stdout}"
    )
...
def test_debug_check_failure(setup_and_cleanup):
    ...
    assert "[ERROR] Output validation failed. Please review your changes." in result02.stdout, (
        f"Expected error details but got {result02.stdout}"
    )
