...
# from utils import DataLoader, DataSplitter
...
def cv(args):
    ...
    for i, fold in enumerate(subsets):
        ...
        # transformers
        transformers = [ getattr(dc.trans, t)(transform_X=True, dataset=train_set) for t in params["dataloader"]["transformers"]]
        ...
        # loss = model.fit(train_set, nb_epoch=params['training']["nb_epoch"], callbacks=validation)
...
#         model.restore(model_dir=params['training']['model_dir']+'/fold_'+str(i)+'/validation')
...
def train(params, datasets):
    ...
    validation = dc.models.ValidationCallback(dataset=valid_set, 
                                              ...
                                              interval=params['training']['valid_interval'], metrics=metrics,
                                              save_metric=params['training']['metrics'].index(params['training']['valid_metric']),
                                              save_dir=params['training']['model_dir']+'/train', 
                                              transformers=transformers, save_on_minimum=save_on_minimum
                                              # output_file=open(params['training']['model_dir']+'/train.log', 'a')
                                             )

    
    # loss = model.fit(train_set, nb_epoch=params_opt['training']["nb_epoch"], callbacks=validation)
    # loss = model.fit(train_set, nb_epoch=params['training']["nb_epoch"], callbacks=validation)
    # loss = model.fit(train_set, nb_epoch=300, callbacks=validation)
    # model.fit(train_set, nb_epoch=params['training']["nb_epoch"])

    for i in tqdm(range(params['training']["nb_epoch"])):
        print('[epoch '+str(i+1)+'/'+str(params['training']["nb_epoch"])+']')
        loss = model.fit(train_set, nb_epoch=1, callbacks=validation)
        print('loss=',loss)
#     model.fit_generator(generator, callbacks=validation)
...
# args = parser.parse_args(args=['-c', 'hyperopt.yaml', '-v'])
...
if args.optimize:
    ...
    for k,v in search_space.items():
        if v.name == 'switch':
            ...
            # print(params['hyperopt']['params'][k], best[k], params_opt['hyperopt']['params'][k])
        elif v.name == 'float':
...
df.to_csv(params['training']['model_dir']+'/train/predict.csv', index=False)
